...
gradient $\nabla F$ are modeled as two explicit subclasses — `NeighborSeed` /
`NeighborFlowMap` and `AuxiliarySeed` / `AuxiliaryFlowMap` — rather than
inferred at runtime from the dataset's dimensions. The type carries the
information; nothing sniffs for a `displacement` dim to decide what to do.
...
- **Auxiliary** (`Auxiliary*`): each grid point carries four arms —
  ...
  arm and no diagonals, and no stored `dx`/`dy`: the stencil *is* the reference
  positions. This decouples the gradient step (`aux_separation_m`) from the seed
  resolution.
...
step but the advection is a method call on one of our objects:
...
each Haller quantity enters. `fm.to_seed()` closes the loop, dropping the
advected positions, `t0` and `T` to recover a time-free seed for re-release.
