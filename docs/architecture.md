...
This document explains why the diagnostic layer in
...
why the numbers that come out of these types are right. That covers the local
east/north frame each separation is measured in, the units the tuning
parameters are stated in, and the well-definedness guard. Symbols live in
...
A `SeedGrid` carries no time. Ingest (`pset_to_flowmap`) is given both the release
time $t_0$ and the end time $t_1$. The signed window $T = t_1 - t_0$ is
...
### The `SeedGrid` and `FlowMap` families are siblings
...
nothing to Parcels. A `SeedGrid` is not a kind of `FlowMap`, because it has no
advected positions and no window. Neither base inherits from the other, and
no common base sits above both, because the two families share almost
nothing. The separation helpers are module-level functions
...
($x_0$). It carries no $t_0$, no $T$, no advected positions, and no data
variables. A `FlowMap` adds the advected positions `lon`/`lat` (the flow map
image $F(x_0)$, and its only data variables) plus the scalar coordinates `t0`
and signed `T`.

Each class *wraps* an `xr.Dataset`, held in `.ds`, by composition, and none
...
seed). They are also linked by the two crossing methods
`SeedGrid.pset_to_flowmap` and `FlowMap.to_seed`. A new pair is therefore a
`SeedGrid` subclass and a `FlowMap` subclass that name each other and fill in
the seams below, and nothing else changes.
...
reported at are arranged. The layout decides how a field can be read between them.
...
- **Neighbour stencil, rectilinear layout** (`Neighbor*`): the stencil is the
  neighbouring grid points $(i \pm 1, j \pm 1)$. It has no dims beyond `(i, j)`,
  and `lon_0`/`lat_0` equal `lon_grid`/`lat_grid`. This is the SPASSO approach (see
...
- **Auxiliary stencil, rectilinear layout** (`Auxiliary*`): each grid point
  ...
  dim. It stores them explicitly as the reference positions `lon_0`/`lat_0`
  around the grid point. The stencil has no centre arm, no diagonals, and no
  stored `dx`/`dy`, so the reference positions record it instead. The arms
  decouple the gradient step (`aux_separation_m`) from the seed resolution.
...
base-class behaviour. `SeedGrid` shares emit and ingest. `FlowMap` shares the
whole diagnostic chain: the Cauchy–Green tensor
...
base. The auxiliary stencil is differenced against a grid point's own four
arms, never against another grid point. So `deformation_gradient` and
`grid_image` read `displacement` labels and no grid axis, and they are
measurably layout-free. The same points laid out on `(i, j)` and on one flat
dim give bitwise-identical $\nabla F$, $C$, eigenpairs, and FTLE. Restating
those two methods on a fourth sibling would duplicate code that has been
measured to be identical. So the unstructured pair inherits them and
overrides only `_interpolator` and, on the seed side, adds `from_points`.
...
They now share one abstract method, `FlowMap._interpolator(field)`, that
returns a callable. The callable takes an $(n, 2)$ array of `(lon, lat)` and
gives back the interpolated values with a leading axis of length $n$, NaN
where the field does not reach. Two implementations answer it, bilinear along the axes for the
...
The obvious layout gives the diagnostic grid location no name of its own. For
the neighbour stencil it is already `lon_0`/`lat_0`. For the auxiliary one,
...
The accessor is named `_grid` rather than `_c` for "centre". Once the
coordinate pair exists on every class, "centre" is a misnomer for the
neighbour case. That case has no arms to be the centre of. `_grid` names the
point without reference to the auxiliary arms, and that name appears on every
plot axis and in every repr.
...
auxiliary arms. The package stores the value rather than making a consumer
reconstruct it from a convention. It costs one duplicated coordinate, and a
dataset read off disk is then self-describing.
...
because there the type carries information. The neighbour flow map passes its
advected positions through. The auxiliary ones take the centroid of their four
arms, with `skipna=False`, so one lost arm makes the grid point NaN. That
matches what the deformation gradient does.
...
auxiliary flow map the surviving `lon_0` is one stencil arm. That arm sits
about $s$ metres from the grid point the diagnostic describes. Keeping
`lon_0`/`lat_0` alongside the canonical pair would leave a second, wrong label
available on the diagnostics.

`FlowMap.image` returns the reference points it was handed alongside the
positions it mapped them to, and labels them `lon_0`/`lat_0`. They are the
$x_0$ that were mapped rather than diagnostic grid points, and the canonical
pair exists so that one name never covers two quantities. It also drops any
`lon`/`lat`/`lon_0`/`lat_0` coordinate the incoming pair carried, since a
coordinate under one of the four returned names would collide with the
variable built from it. That is exactly what a CF dataset's own `lon`/`lat`
axes do.
...
`flowmap.image()` about what an arrival position is called. The suffix would
also have to be dropped again at the first function that has no `t1` to refer
to.
...
Haller writes as $x_0$ and $F(x_0)$ rather than $x_0$ and $x_1$. It survives
into a chained evolution where the output of one `image` call becomes the
`x_0` of the next.

### Why `_positions` hands out degrees, not metres

`FlowMap._positions` returns the reference and advected positions as
`(lon_0, lat_0, lon, lat)` in *degrees*. The subclass gradients convert to
metres themselves, through `_separation_m`. The earlier version converted
first and handed the stencil out already in metres. That only works if the
whole grid shares one frame to convert into. No such frame exists. A
separation becomes metres only once two points are named. It is then taken in
the local east/north frame of *that pair*. The longitude difference is
wrapped to $[-180, 180]$ and scaled by the cosine of the pair's mid-latitude,
and the latitude difference is scaled by the Earth radius alone. The advected
pair therefore gets its own cosine rather than the reference pair's. That is
exactly the rescaling that makes $\nabla F$ report stretching for a rigid
meridional translation.

The same argument removed the shared projection everywhere else. The package
keeps no `lat_ref`, no standard parallel, and no function that converts a
position (as opposed to a pair) to metres. `_wrap_lon` applies to differences only and
...
direct great-circle problem. That choice means measuring the step it took
reproduces the direction it was given. A great-circle step is the more
accurate arc, but it curves away from the direction field the ODE integrates.
That error accumulates linearly over the hundreds of steps in a line
...
`lon_grid + _wrap_lon(lon - lon_grid)`. That is a difference operation again,
and it is correct for any displacement short of 180 degrees.
...
`shrink_lines`' seed pair. Two same-typed adjacent arguments can be transposed
silently. The call does not raise, and it returns a plausible-looking field
...
argument of its type and has nothing to transpose it with. The cost is that
...
That is consistent with `FlowMap.__init__` already accepting, say, a
...
come back as `deformation_gradient` and `cauchy_green`. Two distinct
quantities under one name collide silently when they are merged into a
`Dataset`.

## Walkthrough: a typical session

A typical session defines a seed, generates a particle set, advects it with
Parcels (external), ingests the result into a flow map, and estimates the
FTLE. Every step but the advection is a method call on a `SeedGrid` or a
`FlowMap`:
...
1. `NeighborSeedGrid.from_axes(lon=lon, lat=lat)` turns two 1-D axes into a seed
   ...
   coordinates only, and carries no time.
...
3. Parcels (external, not driven by this package) builds a `ParticleSet`
...
functions, rather than as methods on `FlowMap`, which stays a
gridded-diagnostics object. Both functions consume a `FlowMap`'s xarray
outputs:

- `ftle_ridge_seeds(ftle)` finds seed points at the FTLE ridge tops, local
  maxima over a neighbourhood above a magnitude floor. Its returned
  `xr.Dataset` carries `lon`/`lat` on a `seed` dim, with attributes recording
  the floor applied and the seed separation `window_m` implies.
...
way `shrink_lines` does. So one rule applies to every layout, rather than a
rolling window on the grids that have axes. See
...
in one `Dataset`. The first plot anyone makes is the curves over that field.
Returning the curves alone would force a second eigendecomposition of the
whole grid to recover a field the call already had. The `(line, point)` and
`(i, j)` dims coexist without conflict. The seeds' own attributes ride along
on it as well, minus their `long_name`, which describes the seed points
rather than the curves. A caller who never sees the intermediate `Dataset`
can still read `lcs.attrs["ftle_threshold"]` and
`lcs.attrs["min_seed_separation_m"]` off the result.
...
`ftle_ridge_seeds`, which is where the rule lives. No direction
argument exists, since the flow map already carries $\mathrm{sign}(T)$, and the
returned dataset announces repelling or attracting in its own `long_name` attributes.
What those parameters mean and why they are stated in the units they are is in
[`docs/numerics.md`](numerics.md).

`hyperbolic_lcs()` is a method while the extraction lives in `tensorlines`.
That means `grids` would import `tensorlines`, which already imports the
separation helpers from `grids`. The two names are therefore imported inside
...
which is the introspection the design rules ban. It would also leave a flow
map on a point set with no seeding function at all.

So one rule applies to every layout. A grid point is a candidate when its FTLE
...
metres, where a great-circle radius $r$ is exactly the chord $2R\sin(r/2R)$.
So the query needs no chart, no branch cut, and no monotonic axis.
...
the equatorward edge and 20 km at the poleward one. The window asked for was
60 km. North of 50 N the window degenerated to one cell and returned
...
over. So it reaches `window_m / 2` from its own grid point, and the closest
two seeds can be is half the value passed in. That is easy to misread, and the
...
still returns seeds, different ones. Breaking changes are the norm here, but
this one would not announce itself. A renamed argument raises `TypeError` and a
...
number is exactly `window_m / 2` and no longer depends on the grid. The ball
test is symmetric, so if two seeds sat within the radius each would be in the
other's neighbourhood, and their values would have to be equal.
Distinct-valued seeds are therefore strictly further apart than half the
window, which is the bound rather than an estimate of it. It is reported
anyway, since the attribute exists to answer the misreading.

On a wide band at `window_m = 15_000` the closest pair sits 12 m beyond
7500 m, one grid cell's worth of overshoot. The bound is tight.
...
everywhere and the local-maximum test stops selecting. What comes back is
every point above the magnitude floor. Since the call does not fail and the
output is a plausible-looking seed set, the package warns rather than raising.
The condition is read off the neighbourhood sizes the query already returned,
so it costs nothing and needs no second notion of grid spacing.

### Why an absolute FTLE floor exists at all

Tuning parameters here are meant to be scale-free, and an absolute FTLE floor
is not. A stretching rate that marks a ridge in a fast flow marks nothing in a
slow one, and the same number retunes with the region, the window, and the
season. So `quantile` remains the default and `ftle_min` has to be asked for
by name.

It exists because some analyses need exactly the property the quantile lacks.
A quantile floor is defined by the field it is applied to. So two runs (a
three-day window against a ten-day one, one region against its neighbour) are
each thresholded at their own top decile. The seed counts come out comparable
however the underlying strain differed. A run whose subject is that
difference needs one threshold held fixed across all of it. A quantile cannot
express that. The two selectors are therefore mutually exclusive, and passing
both raises a `ValueError` rather than following a precedence rule.
"quantile 0.9 and 1e-6 1/s" has no reading a caller is likely to have meant.

## Reprs

`SeedGrid` and `FlowMap` carry terse reprs, defined on the base classes and
reading the `lon_grid`/`lat_grid` accessors. No concrete class overrides
anything, and a future pair gets a correct repr for free. Each base class
writes its own string out rather than sharing a helper. The two differ only
by the flow map's trailing `t0`/`T`, and a shared opener would have to hand
...
They are summaries rather than dataset dumps. `.ds` remains how the dataset
is displayed. A repr that does not look like an xarray repr signals that
these objects wrap a dataset rather than subclass one.
