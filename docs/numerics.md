...
This document is the quantitative half of the design record. It covers the
frame in which separations are measured and the longitude arithmetic that goes
with it. It also covers the units the tuning parameters are stated in and the
guard that decides when $\xi_1$ is a direction rather than noise. The
companion document
...
every quantity used below live in [`docs/notation.md`](notation.md), and its
...
The package uses no projection and no shared frame. A separation is only ever taken
...
reference pair. Each is taken with its own cosine, the denominator with the
mid-latitude of the two release points and the numerator with the mid-latitude
of the two arrival points. Taking each cosine at its own pair's mid-latitude
makes the result the tangent-space Jacobian of the flow map. It is written in
the orthonormal east/north basis at $x_0$ on the input side and in the
orthonormal east/north basis at $F(x_0)$ on the output side. Both bases are
orthonormal, so $C = (\nabla F)^\top \nabla F$, its
...
Jacobian is then the identity, but the physical map is not an isometry,
because a zonal separation held at fixed $\Delta\lambda$ contracts by
$\cos\Phi / \cos\phi$. $\nabla F = \mathrm{diag}(\cos\Phi/\cos\phi,\, 1)$
reports exactly that contraction. The local frame replaced an earlier
equirectangular frame whose single standard parallel sat at the seed centroid.
That earlier frame reported the identity in this case, and its error grew with
meridional excursion, which restricted the package to regional domains of
modest latitude range. See GitHub issue #18.

The remaining approximation is the finite arc. $\Delta y$ carries none of it,
because $R\,\Delta\phi$ is the meridional geodesic exactly. $\Delta x$ is the arc along
...
mid-latitude is the pair's own latitude. What is left is the parallel arc
...
30 N, 18 km at 60 N, and 5.5 km at 80 N. The separation at which it reaches
$10^{-4}$ is 541 km, 180 km, and 55 km at the same three latitudes. For a pair
separated in latitude as well, the midpoint value $\cos\bar\phi$
stands in for the interval mean $(\sin\phi_b - \sin\phi_a)/(\phi_b - \phi_a)$ and
is high by $\Delta\phi^2/24$. Here too, $\Delta\phi$ is the pair's own span.
...
the span used. The arms are the same on either auxiliary layout, so
both read the series at the same span. The neighbour span is not a tunable and
it is not one grid cell either, because `_central_separation_m` differences the
$i + 1$ neighbour against the $i - 1$ one. The span is therefore two cells, and
the series is read at twice the grid spacing. For a neighbour stencil on a
coarse grid the finite-difference truncation of that same two-cell span is the
larger term.

The mid-latitude rule is *exact* for a map that is linear in one tangent frame.
Every stencil therefore reproduces the analytic answer of the synthetic test
flow, with no metric error left. Worst case over the three regions the suite
runs (`reference`, `antimeridian`, `high_latitude`), the largest absolute
deviation of any $\nabla F$ component from the analytic value is 1.5e-14 for
the neighbour stencil. The 1.5e-14 figure is about 20 ulp on components of
order 3, which is round-off. The auxiliary stencil's largest deviation is
3.0e-12. The auxiliary figure is some 4500 ulp. That is too large for
round-off, because it is cancellation in differencing arms a kilometre apart on
a sphere 6371 km across. The unstructured layout carries the auxiliary figure exactly,
...
That precision is what the metre-scale stencil differences need, since every
one of them is a wrap of a quantity already far inside the range.
...
being monotone. The rectilinear classes need that axis to be monotone to read
a field along it. Wrapping the
...
any convention. Every pair the package differences (opposite stencil arms,
...
which is the circular mean for a set spanning much less than a hemisphere. It
returns a value on the anchor's branch, so the mean of four auxiliary arms
...
the branch its seed came in on. The same rule from the previous section
applies here, to a step rather than to a difference.
...
formulae for the endpoint. That approach gives a more accurate arc, but the
integrator is solving $\dot r = \xi_1(r)$, where a step has to stay on the
direction field. A
...
the error is second order in the step rather than absent. On a 45-degree
heading at 60 N it is 0.04 m at a 25 km step and 2.6 m at 100 km.
...
piecewise-linear. The linear interval is the grid spacing on a rectilinear
flow map, and the Delaunay triangles of the grid points on an unstructured
one. $\nabla F$ underneath
...
`min_anisotropy` as a dimensionless eigenvalue ratio. The same call therefore
means the same thing at any resolution and over any window. Stated the way
they are implemented, the two lengths would each depend on something the
caller is not asking about. A window as a cell count depends on grid
resolution, and a line as a step count depends on the step size. `window_m`
needs no conversion at all, being compared against distances on the sphere
directly, and only `line_length_m` is still resolved internally, into a step
count.
...
any field. `ftle_min` is an absolute value in the field's own units, and it
does not mean the same thing on any field. A rate that marks a ridge in a
fast flow marks nothing in a slow one. It exists because a run comparing
windows or regions needs one threshold across all of them. A quantile cannot
express that. The reasoning behind offering both is in
...
same misreading. `line_length_m` bounds the traced arc. The integrator spends
at most `line_length_m / (2 * step_m)` steps per direction, and a line that
terminates earlier is shorter. The returned block is NaN-filled past
termination, so every row has equal length. `window_m` is the *diameter* of
the neighbourhood a seed must be the maximum over, and that neighbourhood
reaches only `window_m / 2` from its own grid point. Two seeds can therefore
sit as close as `window_m / 2`.
...
where a great-circle radius $r$ is exactly the chord $2R\sin(r/2R)$. The
neighbourhood is therefore the spherical one, and not a tangent-plane
approximation of it.
...
replaced. Most of that time is spent assembling the neighbour lists.
...
as the window changes". That argument is correct. $\lambda_2$ grows
exponentially in $|T|$, so a fixed floor tightens as the window lengthens, and
a guard on well-definedness becomes a selector. A stretching-rate floor,
`ftle_min_per_day`, cured that by dividing out $|T|$, but it retunes across flow
regimes instead. That retuning is the direction that bites in practice. The
0.005/day default was calibrated to the mesoscale ocean at a 7-day window.

The comparison below is a paper exercise across three flow regimes, and it rests
on one premise. Each regime is integrated over a window $|T|$ *comparable to
its own* $1/\mathrm{FTLE}$, the window being chosen so the flow has time to
stretch by roughly one e-fold. That premise fixes the three
$(\mathrm{FTLE}, |T|)$ pairs:
...
slow one. There, it would eat nearly half the field. The *ratio* form converts
to a rate through the window. A floor $a_{\min}$ on $\lambda_2/\lambda_1$ is,
for incompressible flow ($\lambda_1\lambda_2 = 1$), a $\lambda_2$ floor of
...
to a perturbation of $C$ scales as the inverse of the *relative* gap between
the eigenvalues. $\lambda_2 / \lambda_1$ therefore decides whether $\xi_1$ is
a direction or numerical noise, and no stretching rate can stand in for it.
Measured: at the default 1.15 a 1% error in $C$ swings $\xi_1$ by about 2
degrees. At a ratio of 1.05 the swing is 6 degrees, and by a ratio of 4 it has
flattened out at about 0.25 degrees.

The default 1.15 is the old `ftle_min_per_day=0.005` behaviour carried over
almost exactly. At the 7-day window of the examples, that rate
corresponded to a $\lambda_2$ floor of 1.0725, which for incompressible flow
...
ocean surface is not incompressible. The Cabo Verde example uses a 5-day
window, with points taken 34 km clear of any coast. There, the flow map's
areal factor $\det \nabla F$ has a median of 1.08 forward and 0.97 backward,
nearly area-preserving in the bulk. It ranges from 0.63 to 2.4 forward and
from 0.045 to 14 backward. The implied divergence reaches 0.08–0.13 /day at
the 99th percentile, against a median FTLE of about 0.13 /day, so it is the
...
grid points against the ratio floor's 0.135%. The 78 points that only it
terminates have a median $\det \nabla F$ of 0.75 and a median ratio of 1.6.
Those points are strongly convergent, with $\xi_1$ well defined. A magnitude
floor cannot distinguish "nothing is stretching here" from "everything is
contracting here", so it preferentially terminates shrink lines inside
convergence zones, where attracting LCS are found. On the forward flow of that
same 5-day case, where the median areal factor is above 1, the two guards
agreed almost exactly. That agreement is why the bias did not surface during
forward-only development. The agreement is a property of the window measured
rather than of forward flow in general. At other windows the two termination
rates differ, in both directions.
