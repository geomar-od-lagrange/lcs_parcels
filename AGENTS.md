...
- **Examples stay current with the package.** Everything under `examples/` must
  ...
  one test per example, skipping the ones whose prerequisites (Parcels, the CMEMS
  subset) are absent. The Parcels-free example therefore runs under
  `pixi run test`; the rest need `pixi run -e examples test-examples`.
