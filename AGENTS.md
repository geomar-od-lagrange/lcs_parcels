...
addressed to one of three readers. Know which one before writing, because most rules
...
- **The inquisitive user** wants to convince themselves the science is right
  *without becoming a developer*. They read the example end to end, display the
  ...
  xarray/CF world, but they are not going to read all of `src/`.
- **The developer** needs the design *decisions*: why the auxiliary grid stores
  ...
  they compose), `docs/numerics.md`, the plans, and the tests. `docs/numerics.md`
  explains why the numbers come out right: the metres frame, the tuning
  parameters, the well-definedness guard.

Rules that fall out of it:

- **Explain the non-obvious choice, but do not re-teach the field.** The inquisitive
...
- **Avoid indirection in examples, but use it freely in library code.** The
...
- **A scope statement is not design rationale.** What the package *is and is
  not* stays in the README. That covers three claims: it contains no Parcels
  code, it emits a particle set and ingests advected positions, and it leaves
  the advection run to you. The trusting user cannot use the tool without it. The cut runs between
...
- **Working correctly is not a feature.** Document behaviour a reader would
  ...
  handled is in fact handled. The antimeridian is the running example.
  *That* longitudes may arrive in any convention is a fact the caller needs.
  *That* differences are wrapped to achieve it is one comment at the wrapping
  site, not a paragraph in four docstrings.
...
- **Math in Markdown uses LaTeX, not unicode.** Write `$\nabla F$`, `$\xi_i$`,
  ...
  **`README.md` is the exception, in both directions.** It is the PyPI landing
  ...
  Check with `readme_renderer`, which is what PyPI uses, and the `build` CI job runs
...
- **A link names the general URL, never the redirect target.** Write
  ...
  no `?version=`. Both were checked against the live site, and the bare
...
- **Keep notation in one place.** Symbols and conventions live in a dedicated
  notation doc, so don't redefine them ad hoc across files.
...
- **Nothing on the site is a maintained second copy.** The home page includes
  ...
  and gitignored. Duplicated *output* is free, but duplicated *effort* is what to
...
- **Plans have a lifecycle.** Design/implementation plans live in `plans/`. Once a
  ...
  not link to plans at all, so no link there can break. A plan that is only
...
- **Notebooks are jupytext-managed.** The `.py` (py:percent) is the source of
  truth, and the paired `.md` and `.ipynb` are generated by `jupytext --sync`. Edit
  only the `.py`, then sync. Commit all three. Commit the `.ipynb` in its
  executed form (run it end-to-end and keep the cell outputs) so the rendered
  example is self-demonstrating on the forge. The `.py` and `.md` stay the
  reviewable source, with no cell outputs. After re-syncing from an edited
  `.py` (which regenerates an `.ipynb` with no outputs), re-execute before
  committing. Execute with
  `pixi run -e examples jupytext --sync --execute <name>.py`, without
...
- **Narrate in markdown cells, not comment blocks.** Use `# %% [markdown]` cells
  for prose between steps, but don't explain the flow with long `#` comment blocks
...
- **Show the essence, cut the scaffolding.** An example exists to demonstrate one
  idea (e.g. how our structures marry a given library), so everything not serving
  ...
  or re-downloading on every run is acceptable in an example, but being longer than
...
- **Explicit over shared, even at the cost of duplication.** Write the same
  ...
  code takes the opposite rule, so see [Audience](#audience).)
- **Prefer vanilla plots.** This governs the plot you *first write*: reach for
  `.plot()` / `.plot.pcolormesh()` and accept its defaults. Its defaults label
  axes, titles, and colorbars from the object's name, coords, and attrs. That
  labeling is what output metadata is for. Don't *open* with hand-set titles, axis labels,
...
- **A notebook references only notebooks of lower rank.** Point at the notebook
  ...
  stays linear and no two examples explain each other. The ranks:
...
- **No claimed result you haven't seen.** Never write a summary/conclusion cell
  (or "this shows X" prose) without running the notebook and reading the
  real output first. State what the run produced, not what you expect it to.

## Environments and dependencies (pixi)
...
- **Environments split on weight.** The `default` env stays minimal, holding the
  ...
  3.13, the newest Parcels v4 supports. Run tests with `pixi run test`, and run
  the real examples with `pixi run -e examples ...`. Do not push Parcels/CMEMS/
...
- **Supported versions follow SPEC 0**, which numpy, scipy, xarray, matplotlib,
  and zarr all endorse
  ([SPEC 0](https://scientific-python.org/specs/spec-0000/)). SPEC 0 says to drop
  a Python version 3 years after its release, and a core dependency 2 years
  after its release. The window
  ...
  *above* the SPEC 0 line where a measured failure puts it there, and record the
  measurement next to the floor. It never sits below.
- **Ruff lints and formats everything, and CI gates it.** Run `pixi run lint`
  (`ruff check` plus `ruff format --check`) before handing work over, and `ruff
...
- **Parcels is pinned to a git SHA.** Parcels v4 is alpha, so it is a pypi git
  ...
  the rev deliberately, but don't float it.

## Parcels integration (v4)

Parcels code lives only under `examples/`, never in `src/` (the package contains
no Parcels, and the boundary rules appear later, in Design and architecture).
Notes for writing Parcels examples, verified against the pinned v4 alpha:

- **Currents map onto a FieldSet.** `copernicusmarine_to_sgrid(fields={"U": ds["uo"],
  "V": ds["vo"]})` then `FieldSet.from_sgrid_conventions(sds, mesh="spherical")`.
  v4 has no `from_netcdf`/`from_xarray`/`from_data`. The CMEMS coords
  must carry CF `axis` attrs (T/Z/Y/X).
- **Positions use `x`/`y`/`z`, not `lon`/`lat`.** `ParticleSet(fieldset,
...
- **Lost particles must be recovered to NaN.** By default an out-of-bounds or
  ...
  breaks alignment. That way, losses propagate as NaN through the diagnostics.
...
- **Units are SI.** Return 1/s, metres, seconds, but deviate only for a field with a
...
- **Tuning parameters are scale-free.** Express a knob so its default means the
  ...
  not sufficient, because a floor in 1/day is not scale-free in any useful sense.
  ...
  against one threshold. It is opt-in beside a scale-free default
  (`quantile`). The reason it exists is written down for the developer
...
## Process and change discipline

- **Follow through on findings, but don't triage.** At this scaffolding/design stage
  ...
  findings ("let's do the important ones first") is an antipattern here.
- **PRs land squashed onto a linear `main`.** Merge with squash-and-rebase
  ...
  eventual commit message, `Closes #N` included, and let the merge close
  the issues.
- **Breaking changes are the norm, but they are announced.** This is
  ...
  signatures, data layouts, dim names, or file formats. Never keep backward
  compatibility that complicates the code. No deprecation shims, no compatibility
  aliases, no migration code, no "legacy" branches: delete the old form
  ...
  migration instructions go in the release notes, and no migration code goes in the
  package.
...
- **CalVer, `YYYY.M.D.N`, unpadded.** The version is a date plus a same-day
  ...
  semantic version, because no compatibility contract exists to signal.
  Write the month and day without a leading zero, because that is the form PEP
  440 normalises to. A padded `v2026.08.04.1` would therefore be published
...
- **The tag is the only place a version is written.** `hatch-vcs` derives
  ...
  version literal, so the workflow has no bump commit and nothing to keep in
  sync.
...
- **`CITATION.cff` is the author list, and it must exist in the tagged commit.**
  ...
  contributor list. Anyone not named in it is not an author of the
  ...
  version is written. Do not add a `.zenodo.json`, because it disables `CITATION.cff`
