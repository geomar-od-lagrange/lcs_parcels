...
at basin scale. See "Term growth at basin scale" below.
...
neighbour counts stay constant. That constancy is what growing a real study
does. The scripts were throwaway; the numbers reproduce.

### Cost of `LinearNDInterpolator.__call__`
...
### Warm-starting the simplex search
...
most one or two simplices. Starting from the previous step's simplex and hopping
across the face with the most negative barycentric coordinate settles in one or
...
Three things follow. The KDTree amortisation is nil. It costs 0.003 s to build
against 0.035 s of query per map, so caching the tree saves ~2% of the
windowing and is not worth designing for. The Delaunay is the whole of it. At
example scale the absolute saving is ~9 s over a ten-map session, against ten
Parcels legs over 40 000 particles that run in ~32 s. Caching therefore moves
the diagnostic layer from a third of the run to a fifth, rather than removing
it. Only at $10^6$ grid points does the saving stop being marginal.

The 300 seeds above are ~5x the 58 lines the real Cabo Verde case produces.
Realistic tracing is therefore cheaper. The ratios go up (geometry worth ~40
maps at $1.96\times10^4$), and the absolute savings go down.

### Term growth at basin scale
...
domain at fixed resolution. The window in cells therefore grows, and with it
the neighbour count $k$ grows as the square. Over a fixed 40x40 degree domain with a
...
/ 6.12 / 18.0 s for windows of 7, 13, 27, 55, 109 cells. That scaling is linear
in $w$.
...
particles is a cluster job before any diagnostic runs. It changes memory
...
A bin-and-prune filter keeps the layout-free seeding without the $w^2$. Bin the
points into cells of side $r$ and take per-bin maxima. A point can only be a
...
### Interpolator requirement for `NeighborFlowMap`
...
scattered one onto a `NeighborFlowMap` and tracing tensor lines runs, and
where both are defined the curves agree to 19.8 m median and 0.1 km maximum.

### Termination differences between the two interpolants

- Exact agreement at grid nodes. Between nodes on a regular lattice, barycentric
  is ~40% worse in RMS against a smooth analytic field. Every cell's four
  corners are co-circular, so the diagonal is arbitrary (1582 "/" against
...
Results move. That change is accepted, because this is greenfield code and the
release notes carry it.

## A gradient from the Delaunay ring
...
**No crossover exists in any range anyone will run.** The ring linear fit
...
the idea exists to serve, it is 800x worse in the coarse half. Its error
...
caller cannot set. A **radius-$r$ neighbourhood** puts it back and is second
order in $r$ (4.51e-4 / 1.10e-4 / 2.69e-5 at $r$ = 8 / 4 / 2 km). It makes $r$
worth arms of $s \approx r/1.4$. That variant is defensible at basin resolution
and nowhere else. A 1 km radius finds a median of 0 neighbours at 4.4 km
spacing, 2 at 1 km, and 8 at 556 m. It needs about 1/200 degree before it is
possible at all. Below the point spacing only the arms work, because they are
extra particles.
...
Three further findings hold regardless of what is decided:

- **`_separation_m` is not a general separation helper.** It frames each pair on
  that pair's own mid-latitude. That per-pair frame is exact for a leg that is
  pure east or pure north, and wrong for an oblique one. A ring fit built on it
  therefore cannot reproduce even a linear map (3.6e-5 in `reference`, 4.6e-4 in
  `high_latitude`). Framing every leg of one ring on the centre point instead
  makes the same fit exact to 6e-14. The per-pair frame is right for a stencil
  that differences pairs and insufficient for one that fits over a ring.
...
flow-map contract. That contract runs against the collapse this plan argues
for.
...
Then propagate, as `AGENTS.md` requires:

- `__init__` exports and `__all__`.
- `docs/reference.md` autosummary.
- `docs/api.md` (the three-pair lede, the data model, the constructor section).
- `docs/architecture.md` (the sections "Two axes", "Why the unstructured pair
  subclasses the auxiliary one", and "Why the interpolation seam is abstract on
  `FlowMap`" all change or go, and "Where SciPy sits" needs the new contents).
- `docs/notation.md`.
- `docs/numerics.md` (the piecewise-linear sentence under "Stepping a tensor
  line", plus the scalings above).
- `README.md` scope.
- `CHANGELOG.md`.
- The tests.
...
1. **Where does the cached `Delaunay` live?** It is geometry, so it belongs to
   ...
   a bare dataset has no seed to ask. Three options exist:

   - Build it lazily on the flow map and cache per instance, which rebuilds it
     once per flow map.
   - Cache it on the seed grid and have `pset_to_flowmap` hand it over, which
     works for the normal path and is absent for the hand-built one.
   - Use a module-level cache keyed on the points array, which is a global.

   The amortisation table above says this is not urgent. Per-instance costs
   ~1 s per flow map at example scale and only bites at $10^6$ grid points. The
   per-instance cache is therefore a defensible default, with the seed-grid
   cache as a later optimisation. Confirm that reading, or pick one now.
...
5. **Does `_RESERVED_DIMS` still need `position`** and the rest, once only one
   class exists and possibly a different internal dim for the two position
   components?
6. **What chunk size should the windowing use?** A fixed block, or one sized
   from a memory budget the caller can set?
7. **Should `examples/cabo_verde_unstructured` be renamed?** It demonstrates a
   class that will not exist. Rename it, fold it into `cabo_verde_lcs`, or
   leave it as the point-set example? The `AGENTS.md` notebook-rank list moves
   with the answer.
...
9. **Does the collapse survive $10^8$?** At basin resolution the rectilinear path
   builds nothing and is $O(1)$. The Delaunay is ~21 GiB, roughly an hour, and
   the only one of the three structures that does not tile cleanly. Deleting
...
10. **Does the radius-$r$ ring gradient earn a place at basin scale?** It cuts
    ...
    second order in $r$. It needs about 1/200 degree spacing before a 1 km
    neighbourhood exists, and it wants a well-definedness guard below about
    five neighbours. It is also knife-edged at a radius that lands on lattice
    points. The error is 1.91e-5 at $r$ = 1.5 km against 4.78e-4 at 2.0 km on a
    1 km lattice, because 0.5% variation in $\cos\phi$ flips two neighbours in
    or out.
...
12. **The kd-tree windowing is the more urgent question.** It is already in
    ...
    40x40 degree domain on this machine. Options:

    - Bin-and-prune, which keeps one rule for both layouts, with an $O(n)$
      filter.
    - Chunk the query, which fixes memory and keeps the work.
    - Add `bottleneck` and restore a rolling window for rectilinear grids,
      which is fastest but reintroduces a layout branch.
    - Accept it and require tiling.
