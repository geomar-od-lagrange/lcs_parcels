...
## The headline table does not reproduce

`one-interpolator.md` claims `LinearNDInterpolator.__call__` is 30x to 1620x
slower than the same interpolant computed directly, and its section "How this
arose" rests on that. Measured over 300 000 scattered query points:
...
($156.5 / 0.520 = 301$ and $15617 / 53.27 = 293$), which is what a benchmark
rebuilding the interpolator inside its 300-seed loop produces. Counting the
...
`_interpolator` runs, gives 55x at 19 600 grid points, 247x at 102 400 and 1385x
at $10^6$.
...
## The warm start moves the cost rather than removing it
...
queries, reproduces its NaN pattern at the hull boundary exactly, and its warm
...
## The windowing memory is understated

Resident growth through `neighborhood_maximum` at a 15 km radius measures
0.24 GiB at 102 400 grid points and 2.41 GiB at $10^6$, against the 0.32 GiB of
int64 pair indices the plan quotes, because `query_ball_point` returns Python
lists of Python ints before `np.concatenate` runs. Its 1/50 degree row is
therefore about 38 GiB rather than 5.05 GiB, and the wall arrives one resolution
earlier than its table says.
...
simplex and re-seed when the query count changes, leaving `f(points) -> values`
unchanged, moving neither `image` nor the tensor-line code, and observable
nowhere given the bit-identity above. Item 6 matters more than question 1
concludes rather than less, the build now being the whole cost, and per-flow-map
is still the right default.

## What not to do

Items 4 and 5, the collapse. At its best the scattered path is 6x bilinear at
19 600 grid points, 28x at 102 400 and 275x at $10^6$, the last of those almost
entirely the Delaunay build, which caching amortises to about 29x over a ten-map
session and never removes. Keeping the rectilinear path costs one abstract
method and about 25 lines. The plan retracts the recommendation in its own
opening, its basin-scale section shows the Delaunay is the one structure of the
three that does not tile cleanly, and its question 9 asks whether deleting the
path on the roadmap is wise.
...
`one-interpolator.md` needs reconciling. Its sections "Basin scale changes which
term grows" and "A gradient from the Delaunay ring" stand. "How this arose"
